(function(){
  ...
  /* ---------- Масштабирование десктопной вёрстки под любую ширину экрана
     ---------------------------------------------------------------------
     Весь сайт спроектирован под жёсткий канвас 1920px (абсолютное
     позиционирование, как в Figma). На экранах уже 1920, но шире мобильного
     брейкпоинта (768–1919px: небольшие ноутбуки, планшеты, окно браузера
     не на весь экран) макет раньше просто не помещался — приходилось
     скроллить по горизонтали. Вместо переписывания каждой секции под
     резиновую сетку весь .page масштабируется целиком через CSS zoom —
     это (в отличие от transform:scale) не создаёт новый containing block
     для position:fixed/sticky, так что шапка сайта продолжает корректно
     прилипать к реальному вьюпорту при скролле, просто в уменьшенном виде.
     Ниже 768px и от 1920px и выше — масштаб не применяется, там уже
     штатная мобильная/десктопная вёрстка. */
  (function autoScalePage(){
    ...
    function apply(){
      ...
      page.style.zoom = (w >= MIN_WIDTH && w < BASE_WIDTH) ? (w / BASE_WIDTH) : '';
    }
    apply();
  ...
  if (seatRoot){
    var offSeats = {}; // проданных/недоступных мест на схеме больше нет — все места активны
  ...
  if (repSection){
    ...
    var REP_BTN_TOP_SECTION = 932;   // top кнопки (см. .rep-btn-buy в styles.css)
    ...
    var REP_ACTIVE_IMG_H_MOBILE = 300; // высота центральной картинки на мобильном
    if (repIsMobile){
      REP_REST_W = 130;
      REP_REST_IMG_H = 196;
